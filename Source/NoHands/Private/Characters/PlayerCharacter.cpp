...
{
	if (AASimpleInteractable* SimpleInteractable = Cast<AASimpleInteractable>(InteractionHit.GetActor()))
	{
		if (SimpleInteractable->ActorHasTag("HoldInteract"))
		{
			if (SimpleInteractable->ActorHasTag("Game") && Attributes->GetMoney() > 0)
			{
				APlayable* Playable = CastChecked<APlayable>(InteractionHit.GetActor()); // We have a game in view
...
{
	if (const AASimpleInteractable* SimpleInteractable = Cast<AASimpleInteractable>(InteractionHit.GetActor())) // We have a simple interactable object in view
	{
		if (SimpleInteractable->ActorHasTag("BoostType"))
		{
			APickup* Pickup = CastChecked<APickup>(InteractionHit.GetActor()); // We have a pickup in view
			const EBoostType BoostType = Pickup->GetTypeOfBoost();
			...
			switch (BoostType)
			{
			    ...
			    case EBoostType::EBT_Luck:
					HUDOverlay->SetLuck(Attributes->GetLuck());
				default: break;
			}
		}
	}
...
void APlayerCharacter::InteractionTrace(FHitResult& SphereHitResult)
{
	if (FirstPersonCamera)
	{
		...
		{
			HUDOverlay->DisableInteractPrompt();
			if (LastSeenActor)
			{
				LastSeenActor->DisableHighlight();
...
void APlayerCharacter::SetItemLookedAt(const FHitResult& LookTraceResult)
{
	//Simple Interaction prompt enabler based on what the cursor is tracing over 
	if (LookTraceResult.GetActor()->Implements<UInteractionInterface>())
	{
		TScriptInterface<IInteractionInterface> ThisActor = LookTraceResult.GetActor();

		if (LastSeenActor != ThisActor)
		{
			if (LastSeenActor) LastSeenActor->DisableHighlight();
			LastSeenActor = ThisActor;

			if (ThisActor)
			{
				ThisActor->EnableHighlight();
				HUDOverlay->SetItemName(ThisActor->GetName());
				HUDOverlay->EnableInteractPrompt();
			}
			else HUDOverlay->DisableInteractPrompt();
		}
	}
	else
	{
...
void APlayerCharacter::InitHUDOverlay()
{
	if (const APlayerController* PlayerController = Cast<APlayerController>(GetController()))
	{
		if (const ANoHandsHUD* NoHandsHUD = Cast<ANoHandsHUD>(PlayerController->GetHUD()))
		{
